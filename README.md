...
このツールは、**ローカルファイルパス**に加えて、**Google Cloud Storage (GCS) バケット内のファイル**からでも、処理対象のURLリストを**透過的に読み込む**ことができます。初期の並列処理での負荷や一時的なネットワークエラーに耐えるための**堅牢なリトライ・遅延メカニズム**を備えており、**明確に分離されたDIパイプライン設計**により、高い保守性を実現しています。
...
4.  **堅牢なデータ入力層 (GCSサポート)**:
    * **Go SDK**を利用してGCSパス (`gs://...`) を検知し、Cloud Run Jobやローカル環境で\*\*認証情報（ADC）\*\*を用いてセキュアかつ確実にファイルを読み込みます。
    * 入力ファイルの読み込みロジックは、`pipeline.InputReader`インターフェースによって抽象化され、**GCSとローカルファイルの切り替え**を`pipeline.LocalGCSInputReader`が責務として担います。これにより、入力ソースのテスト容易性と拡張性が最大限に高められています。
5.  **柔軟な設定**: 各フェーズでタイムアウトを設定可能にし、LLM APIキーを環境変数またはCLIオプションで柔軟に設定できます。
6.  **内部設計の最適化**:
...
| **設計パターン** | **依存性注入 (DI) / パイプライン** / **ビルダーパターン** | 処理フロー全体を構造化し、モック化と保守性を向上させます。依存関係の構築は、`pkg/builder`内の専用ファクトリ関数に集約されています。 |
...
| **Stage 3: AIクリーンアップ・出力** | `pipeline.LLMOutputGenerator` | 抽出コンテンツを結合し、**MapReduce**処理（`cleaner.Cleaner`）を実行して最終結果を出力する。 |
...
4.  **出力**: LLMが構造化した最終的なテキスト（Markdown形式）が、`log/slog`と共にファイルまたは標準出力に出力される。
...
### 3\. GCSからの読み込み設定 (クラウド/ローカル)

GCSバケットからファイルを読み込む場合（`-f gs://...`）、プログラムは**アプリケーションのデフォルト認証情報 (ADC)** を使用して認証を行います。

* **Cloud Run Job:** Jobに割り当てられた**サービスアカウント**に、GCSバケットに対する\*\*`Storage オブジェクト閲覧者`\*\*ロールを付与する必要があります。
...
```bash
# 最小実行形式 (ローカルファイルから読み込み)
./bin/llm_cleaner run -f ./urls.txt

# 推奨実行形式 (APIキーとカスタムタイムアウト、並列数を指定)
./bin/llm_cleaner run -k "YOUR_API_KEY" -f ./urls.txt -s 30s -t 3m -p 5

# クラウド運用向け実行形式 (GCSバケットから読み込み)
# JobサービスアカウントにGCS読み取り権限が必要です。
./bin/llm_cleaner run -k "YOUR_API_KEY" -f "gs://my-project-urls/urls.txt"
```
